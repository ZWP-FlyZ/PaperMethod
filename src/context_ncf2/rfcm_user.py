...
fcm_w_out = base_path+'/Dataset/ws/localinfo/user_rfcm_w.txt';
...
class Fcm():
    ...
    def _update_U_imped(self,data,uidx,cent,di=1):
        ...
        for idx in uidx:
            ...
            vidx = np.where(v<0.1);
            v[vidx]=0.1;
#             v = np.divide(1.0,v,out=np.full_like(v, 5),where=v!=0)
            v = (1.0/v)**(1.0/(m-1.0));
            for c in range(clu):
                tt = v[c]**m;
                new_cent[c] += xj*tt;
...
def run():
    ...
    # 1.2-1.3
    rfcm = Rfcm(k,m=1.7,thresdhold=0.2,w=0.85);
